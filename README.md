...
로그인 후 왼쪽의 **상담 작업**에서 귀속연도와 질문을 입력하면 한 건의 상담을 생성합니다. 총수입, 필요경비, 기본공제 인원, 기타 소득공제를 순서대로 확인하고, **내 문서**에 올린 PDF를 수입·경비·공제 체크리스트에 연결하거나 자료가 없는 이유를 기록할 수 있습니다. 입력이 완료되면 계산 결과를 상담에 저장하고, 같은 상담 대화에서 최초 질문을 보내거나 계산 근거를 물을 수 있습니다. 입력값을 고치면 이전 계산 결과는 무효화됩니다.

계산은 선택한 귀속연도 말일 이전에 DB에 등록된 세율·공제 자료를 조회하며, 결과에 실제 사용한 DB 자료의 시행일을 표시합니다. 이는 제한된 입력 항목을 사용하는 **참고 계산**입니다. 서류 금액을 자동 판정하지 않고, 해당 귀속연도의 모든 개정·경과규정·개별 공제 자격이나 실제 신고세액을 확정하지 않습니다. 결과와 연결 법령은 별도로 검토해야 합니다.

구현: `consultation_cases` Alembic 테이블, 사용자별 `/api/consultation-cases` API, 단계별 추가 질문·서류 체크리스트·상담별 계산 스냅샷, React 상담 화면. 기존 대화와 업로드 문서는 삭제하지 않고 상담에 연결합니다.
...
과거 법령 원문은 현행 검색과 분리된 `law_history` 아카이브에 저장합니다.
독립 수집기를 `bash dev/law-history-wsl.sh start`로 실행하면 미수집 버전부터 재개하고,
전체 수집 완료 뒤 원문·조문·부칙 무결성 검수를 자동 실행합니다.
실행 상태·보고서 확인과 시점 적용의 한계는 [과거 법령 관리](docs/LAW_HISTORY.md)를 참고하세요.
...
원문·약칭 정의의 버전 해시 대조, 질문 키워드 필터, 추가 본문 총 4,000자 및 3초 제한을 적용합니다. 추가 조문의 고정 개수 제한은 없습니다.
장애 시 기존 검색 결과를 유지하며 `GRAPH_RAG_ENABLED=false`가 기본값입니다.
2026-09-25 로컬 배포는 `.env`에서 `GRAPH_RAG_ENABLED=true`로 활성화했습니다.
...
법령 종류·시점별 표본 생성, `KG_JUDGE_*` 평가 전용 모델 설정, 건별 진행 저장과 `--resume` 재개 방법도 같은 문서에 있습니다. Judge 결과·혼동 후보 표시를 숨긴 블라인드 인간 검수 양식과 완료본 검증·골드 변환 절차도 제공하며, 변환 결과는 실서비스 관계 승인과 별개입니다.
...
프런트 검증: `cd frontend` 후 `npm test`, `npm run build`. 선택적 Edge headless UI 검증은 서비스 또는 preview 서버를 실행한 뒤 `node tests/profile.browser.cjs`, `node tests/tools.browser.cjs` 및 `node tests/workspace.browser.cjs`로 수행합니다(Playwright 필요, 별도 설치 위치는 `PLAYWRIGHT_MODULE`, 테스트 URL은 `UI_TEST_URL`). 프로필 테스트는 데스크톱·모바일 카드 잘림과 합성 프로필 저장·비밀번호 변경을 검증합니다. 합성 API만 사용하며 실제 사용자 데이터를 변경하지 않습니다.
...
출처 목록의 **조문 제목**은 별도 표시 메타데이터로 취급합니다. 생성 모델이 제목을 잘못 쓰더라도 현행 법령의 `law_articles.article_title`을 법령명·조문번호로 다시 조회해 교정하고, 저장된 조문이 없으면 생성된 제목을 제거합니다. 법적 판단이나 본문 서술은 이 과정에서 재작성하지 않습니다. 스트리밍 화면에도 최종 교정 답변을 반영한 뒤 저장합니다. 과거 법령 답변은 현행 제목으로 덮어쓰지 않습니다.
...
법령·문서 조회가 실패하면 최종 LLM 생성과 인용 보정을 실행하지 않고 판단 유보 안내를 반환합니다.
입력 오류는 대상·번호 확인, 자료 없음은 원문/업로드 확인, 시간 초과·장애는 재시도를 안내합니다.
조회 실패를 규정이나 권리의 부재로 해석하지 않으며 도구 상태와 안내 응답을 함께 저장합니다.

...
별도 검수 게이트가 있는 버전별 조·항·호·목/개념/인용 그래프는
[Knowledge Graph 문서](TAX_KNOWLEDGE_GRAPH.md)를 참고한다. 아래 History* 그래프와 원문 보존 구조는 유지한다.
...
- 연도 자체가 아니라 법령 조회 의도와 날짜/버전 조합으로 history_lookup을 선택한다. `2025년 귀속 종합소득세 계산해줘`, 연도가 있는 계약서 검색은 기존 도구 경로를 유지한다. 계산 필수 입력은 별도로 검증한다.
...
- 조문번호가 있으면 해당 스냅샷의 XML `structure`에서 조·항·호·목을 직접 검증한다. 번호 필드와 내용의 중복 표지는 별도 항으로 취급하지 않는다. 하위 범위와 상위 도입 조건을 함께 제공하고 형제 항·호는 제외한다. 원본 body/hash/벡터는 변경하지 않는다.
- 명시한 법령명과 버전의 공식 법령 ID가 충돌하면 조회하지 않는다. 시행령/시행규칙은 긴 이름을 우선 매칭한다.
- 성공적으로 확인한 법령 ID·버전·스냅샷·기준일·조항 범위를 `chat_logs.message.tools[].history_context`에 저장한다. 후속 조항 질문은 이를 사용하며 생성된 답변 문장에서 시점을 추측하지 않는다. 조회 후 생성 실패나 항 부재여도 확인된 버전 맥락은 유지한다. 새 날짜/법령은 재조회하고 현행 명시·계산·문서 등 주제 변경은 자동 상속하지 않는다. 메타데이터 없는 예전 대화의 구법 후속 질문은 재확인을 요청한다.
...
- 그래프 후보는 같은 기준일의 대상 버전과 항·호·목 존재를 다시 확인한다. 요청 범위 발췌 안에 실제 포함된 인용만 확장하고 원문 스냅샷 해시와 인용 문구도 PG에 대조한다. 추가 근거의 고정 개수 제한은 없으며 생성 입력은 별도 10,000바이트 예산으로 선별한다. 시간 제한/오류 시 역사 기본 근거만 유지한다.
- 답변 자료는 source 번호를 포함한 JSON 10,000 UTF-8 bytes 이내로 구성한다. 질문 대상 근거를 먼저 확보하고 남는 공간에 보충 근거를 배치한다. 필수 범위 자체가 초과하면 자르거나 그래프로 대체하지 않고 범위 축소를 요청한다. 전체 프롬프트의 모델별 토큰 계산과는 다른 방어적 제한이다.
...
- 역사 응답은 검증 완료 후 본문을 한 번에 전달한다. 도구 상태는 SSE로 전송하지만 토큰 단위 생성 스트리밍은 하지 않는다. 조회 후에도 running을 유지하며 생성 및 원문 인용 검증을 모두 통과해야 ok를 보낸다. 실패 시 error와 단계/원인 코드, 조회 성공 여부를 별도로 저장·표시한다. 문자열 인용 검증이 설명의 법적 의미를 인증하지는 않는다.
...
- 완료 후 실제 의미 검색 3건·유보 2건·과거 관련 회귀 테스트 31개 통과, Neo4j 5,400 스냅샷 전수 감사 오류 0. 상세 방법과 한계는 `docs/evaluation/2026-09-25-history-embedding-validation.md`.

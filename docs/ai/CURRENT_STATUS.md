...
- 문구상 관계 라벨과 대상 버전 시점 확인을 분리했다. 인간 라벨 0건이며 양식의 `review`는 모두 비어 있다. 변환은 Neo4j 승인이나 서비스 반영을 수행하지 않는다. 검수 및 비교 절차: `evaluation/KG_RELATION_REVIEW.md`.
- 새 백엔드 이미지를 빌드하되 실행 중인 서비스는 교체하지 않았다. Compose 환경의 일회성 테스트 컨테이너에서 전체 `pytest -q`: 716 passed/2 skipped. 환경설정 없는 단독 이미지 테스트의 기존 DB/모델 설정 의존 실패 3건은 Compose 환경에서 통과했다.

## 2026-09-26 관계 Judge 56건 실행·인간 검수 대기

- `evaluation/sources/kg_relation_stratified.json`의 미검수 56건을 OpenRouter Luna로 2회씩 진단했다. 최초 배치에서 429로 실패/부분 실패한 15건은 새 보고서에서 실패 카드만 재시도했고, 마지막 원문 인용 검증 오류 1건도 재시도했다. 최종 `evaluation/runs/kg-relation-final-20260926.json`은 `advisory_only=56`, 모델 합의는 추출 후보 48건 `supported`, 대상 교환 혼동 후보 8건 `unsupported`다. 이는 인간 정답 대비 정확도가 아니다.
- 완료 보고서의 실패 카드 재시도 `--retry-from`, 요청 간격 `--delay-sec`를 추가했다. 최종 제공자 보고 누적 사용량은 65,245토큰·0.0100509 USD, Judge 시도 144회다. 원본 카드 파일은 Judge 판정이 없는 독립 인간 검수 자료로 보존했다.
- 인간 골드 0건, 시점상 대상 버전·법적 적용 검수 0건. Neo4j 관계 승인·실서비스 RAG 설정은 변경하지 않았다. 검수 인계와 파일 위치: `evaluation/KG_RELATION_REVIEW.md`.

## 2026-09-26 관계 평가 표본·실행 관리

- 법률·시행령·시행규칙에서 과거(2016년 전)·최근(2026-09-26까지 시행) 버전을 자동 선정했다. 6개 법령 56건: 법률 19/시행령 19/시행규칙 18, 과거 27/최근 29, 정의 28/인용 28. 그중 대상 교환 혼동 후보 8건은 미검수다. 동일 입력 재생성 SHA-256 일치, 56건 원본 재대조 통과.
- 관계 Judge에 `KG_JUDGE_*` 전용 provider·모델·추론 수준·입력/출력 예산을 추가하고 카드별 원자적 진행 저장·동일 설정 재개·실패 재시도·제공자 사용량 합계를 연결했다. 실제 OpenRouter Luna 2건 호출에서 두 건 모두 진단 결과와 토큰/비용 수치가 기록됐다. 인간 골드와 시점상 법적 적용 평가는 아직 없다.

## 2026-09-26 관계 전용 LLM Judge 평가 경로

- 미검수 관계 카드의 정확한 원본 행·해시·관계 키를 PostgreSQL/XML에서 재구성한 뒤에만 LLM에 관계의 명시 여부를 묻는 진단 경로를 추가했다. 답변용 Judge와 분리했고, 원문 인용 강제·입력 예산 제한·반복 판정 합의·오류 시 판정 유보를 적용한다.
- 인간 확정 라벨과 별도로 비교하는 채점기를 추가했다. gold 0건 상태에서는 관계 정확도 수치를 산출하지 않는다. Judge 결과는 Neo4j 승인 상태·채팅 검색 설정·기존 평가 정답셋을 변경하지 않는다. 실행법과 제한: `evaluation/KG_RELATION_REVIEW.md`.

## 2026-09-26 과거 법령 검수기·관계 평가 준비

- 버전 3162·3163 제125조의 XML/본문 불일치 2건은 원본 손상이 아니라 동일 조문번호의 개정 지시문·조문 본문을 마지막 행 하나와 비교한 검수기 오탐으로 확인했다. 원본 행 ID별 대조로 수정하고 회귀 테스트를 추가했다. 전체 5,400개 버전 재검수에서 해시·파싱 오류 0, XML/본문 불일치 0이다. 원본 DB와 Neo4j는 변경하지 않았다.
- 법령 관계 검수 카드 생성기와 검수 규약을 추가했다. 초기 13건은 모두 `unreviewed`이며 정답 라벨·전문가 승인은 아직 0건이다. `evaluation/KG_RELATION_REVIEW.md` 참조.

## 2026-09-25 과거 법령 Knowledge Graph 전수 적재·감사

- 수집 완료 스냅샷 5,400/5,400개를 Neo4j에 적재하고 버전별 조·항·호·목 노드 6,924,423개, 고유 명시적 정의·인용 관계 1,313,901개를 생성했다. `audit`에서 누락/초과 체크포인트 0, 기대/실제 수량 일치를 확인했다.
- 조문이 없는 개정 부칙·별표 중심 버전 1개는 0건 조문 체크포인트로 처리했다. 같은 번호가 반복된 101개 스냅샷의 구조 단위 1,734개는 발생 순서로 분리해 덮어쓰기를 보정했다.
- 당시 검수기는 해시·파싱 오류 0, XML 발췌/저장 본문 불일치 2건(버전 3162·3163 제125조)을 보고했다. 위 2026-09-26 재검수에서 2건 모두 조문번호 기준 비교로 인한 오탐으로 확인·수정됐다. 중복 조문번호가 있는 자료는 승인·검색에서 모호한 원문 선택을 계속 차단한다.
- 승인된 관계는 시범 3개, 나머지 1,313,898개는 candidate로 남아 과거법령 RAG에 자동 투입하지 않는다. 실제 세무 의미·적용시점의 전문가 검수 및 정답률 평가는 별도다. 상세 결과: `docs/TAX_KNOWLEDGE_GRAPH.md`.

## 2026-09-25 버전별 Knowledge Graph 시범 적용 당시 기록 — 위 전수 적재로 확장

- 보존 XML에서 버전별 조·항·호·목 노드와 `CHILD_OF` 계층을 구성하고, 명시적 정의·정식 법령명 인용을 출처 문구/해시가 있는 검수 후보로 저장한다. 검수된 관계만 과거 법령 RAG 조회에 연결했다.
- 소득세법 버전 306 `제1조의2` 한 조문으로 시작했다. 조·항·호 노드 8개, 정의 개념 5개, 명시적 참조 2개, 후보 관계 7개 중 원문 일치를 확인한 정의 2개·인용 1개를 승인했다. 이 단계의 시범 적재는 위의 전수 백필로 확장됐지만 법률가의 법적 의미 검수는 여전히 아니다.

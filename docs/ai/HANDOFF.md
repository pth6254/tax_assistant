...
## 2026-09-25 버전별 Knowledge Graph 후속

- 구현·시범 적용 범위와 검수 명령은 `docs/TAX_KNOWLEDGE_GRAPH.md`. 소득세법 버전 306 `제1조의2`만 적재하고 3개 관계를 원문 일치 검수했다. 다른 4개 관계 후보와 나머지 법령 버전은 미검수·미적재다. 이를 전체 법령에 대한 검수 완료라고 표시하면 안 된다.
- 과거법령 전 범위의 조·항·호·목 적재는 별도 배치/체크포인트·처리량 계획과 데이터 변경 범위 확인이 필요하다. 검수자 권한/이중검수, 누락·오탐 계량, 정의 문형 확장, 관계 골든셋 평가도 미완료다.

...
const TABS = [
  { key: 'income',      label: '소득세' },
  { key: 'capital',     label: '양도소득세' },
  { key: 'inheritance', label: '상속세' },
  { key: 'gift',        label: '증여세' },
]
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          ...
          <form onSubmit={handleSubmit} style={{
            ...
            {error && (
              <div style={{ fontSize: 12, color: 'var(--danger)', padding: '8px 12px', background: 'rgba(255,92,92,.08)', borderRadius: 8 }}>
                {error}
              </div>
            )}
  ...
  return (
    <div>
      <label style={labelStyle}>
        {field.label}
        {field.required && <span style={{ color: 'var(--accent2)', marginLeft: 2 }}>*</span>}
        {field.hint && <span style={{ marginLeft: 6, fontWeight: 400, opacity: 0.7 }}>({field.hint})</span>}
      </label>

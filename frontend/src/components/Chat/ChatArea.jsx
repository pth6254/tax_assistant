...
export default function ChatArea({ user, conversationId, conversationTitle, onMessageSent, onOpenCalculator, pendingQuestion, onPendingQuestionConsumed, onCreateConversation, onRevisionCreated, library }) {
  ...
  useEffect(() => { setRevisionError('') }, [conversationId])
  const revise = async query => {
    const last = messages.at(-1), source = conversationId
    if (loading || historyLoading || revisionBusy.current || !last?.message_id) return
    revisionBusy.current = true; setRevising(true); setRevisionError('')
    try {
      const fork = await reviseConversation(source, last.message_id, query)
      if (active.current === source) onRevisionCreated?.(fork)
    } catch (e) {
  ...
  useEffect(() => {
    if (pendingQuestion && conversationId && !historyLoading && !historyError && !loading &&
        (typeof pendingQuestion === 'string' || pendingQuestion.id === conversationId)) {
      send(typeof pendingQuestion === 'string' ? pendingQuestion : pendingQuestion.query); onPendingQuestionConsumed?.()
    }
    ...
    <main className="chat-main">
      ...
      <div className="messages-scroll" ref={scroller} onScroll={() => { const el = scroller.current; follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 90; if (follow.current) setUnseen(false) }}>
        <div className="messages-content">
          ...
          <Notice>{revisionError}</Notice>
          {revising && <p role="status">원래 대화를 보존하고 새 답변을 준비합니다…</p>}
